...
const parseTypeArg = typeArg => {
  ...
  if (['runCommands', 'runCommand', 'command', 'commands'].indexOf(typeArg) !== -1) {
    type = 'runCommands';
...
const listDefinitions = (options, gorc, typeArg) => {
  ...
  const definitions = _.has(gorc, type) ? { [type]: _.get(gorc, type) } : gorc;
  ...
  const columnOptions = {
    ...
    config: {
      directory: {
        dataTransform: data => data.replace('/home/$USER', '~')
      },
      aliases: {
        dataTransform: data =>
          data
